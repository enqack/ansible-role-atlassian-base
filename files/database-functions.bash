...
download_mysql_driver() {
  ...
  if [ ! -f "$1/$driver-bin.jar" ]; then
    ...
    curl -L -s http://dev.mysql.com/get/Downloads/Connector-J/$driver.tar.gz | tar zxv -C /tmp
    cp /tmp/$driver/$driver-bin.jar $1/$driver-bin.jar
  fi
}

download_postgresql_driver() {
  local driver="postgresql-9.3-1103"
  if [ ! -f "$1/$driver.jdbc4.jar" ]; then
    echo "Downloading Postgresql JDBC Driver..."
    curl -L -s https://jdbc.postgresql.org/download/$driver.jdbc4.jar | tar zxv -C /tmp
    cp /tmp/$driver/$driver.jdbc4.jar $1/$driver.jdbc4.jar
  fi
}
